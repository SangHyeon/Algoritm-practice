	...
	for(test_case = 1; test_case <= T; test_case++) {
        ...
        while(1) {
            ...
            for(int x=0; x<N; x++) {
                for(int y=0; y<N; y++) { 
                    if(table[x][y] == cnt) {
                        if(x>=0 && x<N && y>=0 && y<N && x+1<N && table[x+1][y] == -1) {
                            table[x+1][y] = cnt+1;
                            flag = false;
                        }
                        if(x>=0 && x<N && y>=0 && y<N && x-1 >=0 && table[x-1][y] == -1) {
                            table[x-1][y] = cnt+1;
                            flag = false;
                        }
                        if(x>=0 && x<N && y>=0 && y<N && y+1 <N && table[x][y+1] == -1) {
                            table[x][y+1] = cnt+1;
                            flag = false;
                        }
                        if(x>=0 && x<N && y>=0 && y<N && y-1 >=0 && table[x][y-1] == -1) {
                            table[x][y-1] = cnt+1;
                            flag = false;
                        } 
                    }
